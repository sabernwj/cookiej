...
class DisplayContent {
    ...
    for (int i = 0; i < textsList.length; i++) {
      ...
      if (i < matchs.length) {
        ...
        if (_singleText.contains(RegExp(userRegexStr))) {
          _displayContentList
              .add(DisplayContent(ContentType.User, _singleText));
          continue;
        }
        //话题
        if (_singleText.contains(RegExp(topicRegexStr))) {
          _displayContentList
              .add(DisplayContent(ContentType.Topic, _singleText));
        }
        //url链接
        if (_singleText.contains(RegExp(urlRegexStr))) {
          ...
          if (_singleText.contains(RegExp('http://m.weibo.cn/'))) {
            _displayContentList
                .add(DisplayContent(ContentType.WeiboWebLink, '查看'));
          } else {
            var urlInfo = await UrlRepository.getUrlInfo(_singleText) ??
                UrlInfo(annotations: []);
            ...
            try {
              if (urlInfo.annotations.length != 0 &&
                  urlInfo.annotations[0] != null) {
                switch (urlInfo.annotations[0].objectType) {
                  case 'place':
                    contentType = ContentType.Place;
                    displayText =
                        '\u{f124}' + urlInfo.annotations[0].object.displayName;
                    break;
                  case 'video':
                    ...
                    displayText =
                        '\u{f03d}' + urlInfo.annotations[0].object.displayName;
                    hasVideo = true;
                    break;
                  case 'collection':
            ...
            _displayContentList
                .add(DisplayContent(contentType, displayText, info: urlInfo));
          }
        }
      }
    }
  ...
  static String formatDisplayContentsToStr(
      List<DisplayContent> displayContents) {
    return '';
  }
}
