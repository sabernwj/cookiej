...
class ShowImagesView extends StatefulWidget {
  ...
  ShowImagesView(this.imgUrls, {this.currentIndex = 0, this.heroTag})
      : pageController = PageController(
            initialPage:
                currentIndex); //imageList=<PhotoViewGalleryPageOptions>[];
  @override
...
class _ShowImagesViewState extends State<ShowImagesView>
    ...
    showWidget = isInPop
            ...
            child: Hero(
                ...
                child: Image(
                  image: PictureRepository.getPictureFromUrl(
                      widget.imgUrls[widget.currentIndex],
        ...
        : ExtendedImageGesturePageView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              Widget image =
                  buildImage(url, index, sinaImgSize: SinaImgSize.mw1024);
              return image;
            },
  ...
  Widget buildImage(String url, int index,
    ...
    url = PictureRepository.changeUrlImgSize(url, sinaImgSize);
    Size rawImageSize;
    ...
    var _initGestureConfigHandler = (ExtendedImageState state) {
      rawImageSize = Size(state.extendedImageInfo.image.width.toDouble(),
          state.extendedImageInfo.image.height.toDouble());
      if ((rawImageSize.width / rawImageSize.height) < (9 / 21))
        isLongPicture = true;
      double scaleWithScreen = MediaQuery.of(context).size.height /
          ((rawImageSize.height / rawImageSize.width) *
              MediaQuery.of(context).size.width);
      double realScale = rawImageSize.width / MediaQuery.of(context).size.width;
      if (isLongPicture) {
        scaleWithScreen = MediaQuery.of(context).size.width /
    ...
    var _onDoubleTap = (state) {
      ...
      Offset pointerDownPosition =
          isLongPicture ? Offset.zero : state.pointerDownPosition;
      if (isLongPicture) {
        needRealScale = false;
        scaleWithScreen = MediaQuery.of(context).size.width /
            ((rawImageSize.width / rawImageSize.height) *
    ...
    Function _loadStateChanged = (ExtendedImageState state) {
      ...
      if (state.extendedImageLoadState == LoadState.loading) {
        var preImg = ExtendedImage(
          image: PictureRepository.getPictureFromUrl(url,
              sinaImgSize: SinaImgSize.bmiddle),
          initGestureConfigHandler: _initGestureConfigHandler,
          heroBuilderForSlidingPage: (widget) => widget,
          fit: BoxFit.contain,
        ...
        return Stack(
          ...
          children: <Widget>[
            preImg,
            Center(
              child: (state.loadingProgress == null || !isShowLoadingProgress)
                  ? Container()
