...
class WeiboListAsyncViewModel extends AsyncViewModel {
  final List<WeiboWidgetVM> dataList = [];

  Future<void> startLoad() async {
    dataList.clear();
    dataList.addAll(await getData(GetDataType.StartLoad));
    changeState(AsyncViewModelState.Complete);
  }
...
class WeiboListAsyncWidget extends AsyncViewWidget<WeiboListAsyncViewModel> {
  final RefreshController _refreshController =
      RefreshController(initialRefresh: false);

  @override
  Widget buildIdleWidget(BuildContext context, WeiboListAsyncViewModel vm) {
    vm.startLoad();
    return super.buildIdleWidget(context, vm);
  }

  @override
  Widget buildCompleteWidget(BuildContext context, WeiboListAsyncViewModel vm) {
    return SmartRefresher(
      enablePullDown: true,
      enablePullUp: true,
      header: ClassicHeader(
...
class WeiboListGetXWidget extends StatelessWidget {
  final RefreshController _refreshController =
      RefreshController(initialRefresh: false);
...
class WeiboListGetXVM extends GetxController {
  final List<WeiboWidgetVM> dataList = [];
